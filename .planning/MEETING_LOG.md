...
### 회의 2026-04-30-ζ-5: 카탈로그 콘텐츠 정의 변경 + 디폴트 무게 시스템 + SEED-003 보류 (2026-04-30)

**참석:** 대표(임주용 — 페르소나 예시 + 4 질문 던짐), Claude(평가자/구현자), 자동 소환 자문단:
- 프로덕트: Pajak 의장 (UX 흐름 / "≤ 4 카드" 룰)
- 소비전략: 박충환 / Nir Eyal (Hooked)
- 마케팅: 김경록 (일헥타르)
- 데이터: 이화식
- 운동과학: 한체대 교수 / 운동생리학자
- 페르소나 4명
...
**배경:**
회의 ζ-4 결정(웨이트 카탈로그 5개 = 분할×강도 매트릭스)이 대표 vision 과 어긋남. 대표 페르소나 예시 2건(♂40세/fat_loss, ♀27세/muscle_gain) + 스크린샷 7개(여름 다이어트, 생리주기, 상급자 등, 어깨 회피, 거북목, 휴가 7일, 내 스펙) 제시. 또 디폴트 무게/세트 시스템 + 1세트 = 빈바(명칭 X) 디자인 룰 신규 도입.

**근본 결정 4건 (4 질문 컨펌):**

**1. ROOT 3픽 유지 (단 온보딩 선호로 시각 정렬·강조)**
- 자문단 다수(Pajak·이화식·김경록 + 페르소나 3명) ROOT 유지 권장. 사유:
  - ROOT = "오늘 뭐 할까" 모드 게이트 (멀티 모드 사용자 일상 진입)
  - "≤ 4 카드" 룰 보존 (한 화면 카드 12+개 방지)
  - 정체성 시각화 ("오운잘 = 웨이트·러닝·홈트")
- 변경: 온보딩 `workout_preference` 결과로 ROOT 카드 정렬·강조. 선호 안 한 모드는 dim 처리.

**2. 룰엔진 + 카탈로그 하이브리드**
- 코드 검증 결과(workoutEngine.ts L647-727): goal 분기는 일치(fat_loss=15-20, muscle_gain=8-12, strength=3-5)지만 **누락 3건**:
  - 1세트 = 빈바, 2세트~ 본격 무게 분기 없음
  - 덤벨 default ♂10·♀5 (대표 의도 ♂3·♀2 와 불일치)
  - onboarding goal 4종(fat_loss/muscle_gain/endurance/health)에 strength 입력 path 없음
- 결정: **룰엔진은 운동 풀·구성·카디오 담당. 세트별 무게·횟수는 카탈로그 setTemplate 가 정의.**
  - 카탈로그 SSOT: 운동 role(compound/accessory/isolation/light)별 setTemplate 배열 (1세트, 2세트, 3세트~)
  - 룰엔진은 fallback only. 덤벨 ♂3·♀2 는 카탈로그에서 명시.

**3. 부위별 카드 = 그날 단발 운동 (장기 플랜 X)**
- 대표 정정: "부위별은 장기 플랜말고 그날 그냥 하고싶을때 하는거. 5분할도 보면 주 5회로 진행되는 만큼 장기 말고 그날 운동으로 생각. 그날 그 부위 하고싶을 때 선택일 확률 99.9%."
- 결정: 부위별 1 카드 클릭 → 5개 부위 선택 → **그날 단일 세션** (1회성). 5분할 12주 장기 X.

**4. 프로그램 기간: 4주 ~ 16주 + 4주 청킹**
- 운동과학 자문단: 다이어트 12-16주(3-4개월) 한국 헬스 시장 일반적
- Nir Eyal·박충환: 16주 길어 이탈 위험. 단 청킹 + 재후킹으로 완화 가능
- 결정: **웨이트·홈트 = 4주~16주 (4w×4 chapter)**. **러닝 = 12주 cap 유지** (회의 ζ-4 결정).
- 메모리 룰 갱신: `feedback_user_attention_span.md` — 12주 상한 → 16주 (러닝 제외)

**SEED-003 (YouTube 시장 조사) — 보류:**
- 대표 결단: "조사는 넘기고". SEED-003 status = paused.
- 카탈로그 콘텐츠는 Claude 도메인 지식(운동과학 자문단 + 한국 헬스 트렌드)으로 빠르게 1차 안 작성 → 대표 검토 → 정적 SSOT 박음.
- YouTube quota 절감 코드(commit b9bb87c)는 잔존. 추후 SEED-003 재개 시 즉시 활용.

**카탈로그 콘텐츠 정의 (회의 ζ-4 → ζ-5 갱신):**

이전(ζ-4): 5개 (분할 × 강도 매트릭스)
신규(ζ-5): **부위별 1 + 목적별 N + 캠페인 N — 한 페이지 ≤ 4 카드 큐레이션**

| 카드 종류 | 노출 조건 |
|---|---|
| 부위별 (중·상급자용) — 그날 단발 | 모든 사용자 |
| 목적별 프로그램 (3-4개) | onboarding goal 매칭 (fat_loss/muscle_gain/health) |
| 캠페인성 (시즌·이벤트·여성) | 조건부 (시즌 / gender / 헬스 경험) |

**페르소나 매칭 룰** (대표 예시):
- ♂40세 173/78 fat_loss → 부위별 / 여름 다이어트 12주 / 급빠 4주 / HIIT
- ♀27세 152/41 muscle_gain → 부위별 / 8주 근육 / 3개월 인바디 D / 2분할 체력

"운동 목적 다르게 선택" UI = 상단 칩 탭 (다이어트/근력향상/건강).

**작업 순서 (회의 ζ-5 후속):**

- **즉시 (이 회의 마무리)**: 카탈로그 SSOT (`src/constants/programCatalog.ts`) 1차 안 작성
- **후속**: WeightHub UI 구현 + ChatHome 완전 폐기 (SEED-002 P3)
- **후속**: setTemplate → 룰엔진 입력 흐름 연결 (워크아웃 세션에 1세트 빈바 적용)
- **후속**: 4주 청킹 + 챕터 재후킹 UX 적용

**의도적 비결정:**
- 카탈로그 콘텐츠 최종 목록 (1차 안 후 대표 검토 → 추가/제거)
- "내 스펙 맞춤 플랜" (AI 어드밴스) — 별도 트랙
- 캠페인 자동 노출 로직 (시즌 / gender 분기) — P3 시작 시 결정

**관련:**
- SEED-002 (ProgramFirst UX 전환) — 카탈로그 콘텐츠 정의 갱신
- SEED-003 (YouTube 시장 조사) — paused
- 메모리 룰: `feedback_user_attention_span.md` 갱신 (12주 → 16주, 러닝 제외)
