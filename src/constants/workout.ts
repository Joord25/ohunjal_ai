...
export const LABELED_EXERCISE_POOLS: { label: string; keywords: string[]; exercises: string[] }[] = [
  ...
  { label: "후면 어깨", keywords: ["후면", "rear", "리어"], exercises: ["케이블 페이스 풀 (Cable Face Pulls)", "리어 델트 플라이 (Rear Delt Fly)", "밴드 풀 어파트 (Band Pull-Aparts)", "리버스 스노우 엔젤 (Reverse Snow Angel)", "Y-T-W 레이즈 (Y-T-W Raises)", "사이드 라잉 외회전 (Side-Lying External Rotation)", "케이블 외회전 (Cable External Rotation)", "스탠딩 외회전 (Standing External Rotation)", "스탠딩 케이블 외회전 (Standing Cable External Rotation)", "밴드 외회전 (Band External Rotation)"] },
