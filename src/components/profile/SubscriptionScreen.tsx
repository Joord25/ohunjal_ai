...
export const SubscriptionScreen: React.FC<SubscriptionScreenProps> = ({ user, onClose, initialStatus, onCancelFlowChange }) => {
  ...
  const tierKrwAmount = tier ? KRW_BY_TIER[tier] : 4900;
  const tierUsdAmount = tier ? USD_BY_TIER[tier] : 4.99;
  const tierPriceLabel = tier ? formatPriceForLocale(tier, locale === "ko" ? "ko" : "en") : (locale === "ko" ? "₩4,900" : "$4.99");
