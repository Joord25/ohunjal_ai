...
export const Onboarding: React.FC<OnboardingProps> = ({ userName, onComplete }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-white relative overflow-hidden">
      ...
      {/* Welcome */}
      {step === "welcome" && (
        <div className="flex-1 flex flex-col bg-[#1B4332] relative overflow-hidden">
          {/* Decorative background shapes */}
          <div className="absolute inset-0 overflow-hidden">
            <div className={`absolute -top-20 -right-20 w-64 h-64 rounded-full bg-[#2D6A4F]/40 transition-all duration-1000 ${welcomeVisible ? "scale-100 opacity-100" : "scale-50 opacity-0"}`} />
            <div className={`absolute top-1/4 -left-16 w-48 h-48 rounded-full bg-[#2D6A4F]/30 transition-all duration-1000 delay-200 ${welcomeVisible ? "scale-100 opacity-100" : "scale-50 opacity-0"}`} />
            <div className={`absolute bottom-1/4 -right-10 w-32 h-32 rounded-full bg-[#40916C]/20 transition-all duration-1000 delay-400 ${welcomeVisible ? "scale-100 opacity-100" : "scale-50 opacity-0"}`} />
            <div className={`absolute bottom-20 left-8 w-20 h-20 rounded-full bg-[#52B788]/15 transition-all duration-1000 delay-[600ms] ${welcomeVisible ? "scale-100 opacity-100" : "scale-50 opacity-0"}`} />
          </div>

          {/* Decorative icons */}
          <div className="absolute inset-0 overflow-hidden">
            <svg className={`absolute top-[15%] right-8 w-10 h-10 text-[#52B788]/30 transition-all duration-700 delay-[500ms] ${welcomeVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
            </svg>
            <svg className={`absolute top-[20%] left-10 w-8 h-8 text-[#52B788]/20 transition-all duration-700 delay-[700ms] ${welcomeVisible ? "opacity-100 rotate-0" : "opacity-0 -rotate-45"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
            </svg>
            <svg className={`absolute bottom-[35%] left-6 w-9 h-9 text-[#52B788]/25 transition-all duration-700 delay-[800ms] ${welcomeVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-8 relative z-10">
            ...
            <button
              onClick={() => goTo("gender")}
              className={`w-full py-4 rounded-2xl font-bold text-[#1B4332] bg-white active:scale-95 transition-all duration-700 delay-[600ms] shadow-lg ${welcomeVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
            >
